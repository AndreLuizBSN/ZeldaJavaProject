...
public class AStar {
	...
	private static Comparator< Node > nodeSorted = new Comparator< Node >() {
		@Override
	...
	public static List< Node > fintPath( World world, Vector2i start, Vector2i end ) {
		...
		while ( openList.size() > 0 ) {
			Collections.sort(openList, nodeSorted);
			...
			//valida tudo ao redor do objeto no mapa
			for ( int i = 0; i < 9; i++ ) {
				if ( i == 4 ) continue;
				...
				if ( vectInList(closedList, a) && gCost >= current.gCost ) continue;
				
				if ( ! vectInList(openList, a) ) {
					openList.add(node);
	...
	private static boolean vectInList( List< Node > list, Vector2i vector ) {
